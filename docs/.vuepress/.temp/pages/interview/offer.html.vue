...
<p>拷贝一个字符串，会新开辟一个存储地址，切断了对象的指针联系</p>
...
<p>父组件beforeCreate-&gt;父组件created-&gt;父组件<code v-pre>beforeMount</code>-&gt;子组件beforeCreate-&gt;子组件created-&gt;子组件beforeMount-&gt;子组件mounted-&gt;父组件mounted</p>
<ol start="2">
<li>子组件更新过程</li>
</ol>
<p>父组件beforeUpdate-&gt;子组件beforeUpdate-&gt;子组件updated-&gt;父组件updated</p>
<ol start="3">
<li>父组件更新过程</li>
</ol>
<p>父组件beforeUpdate-&gt;父组件update</p>
<ol start="4">
<li>销毁过程</li>
</ol>
<p>父组件beforeDestroy-&gt;子组件beforeDestroy-&gt;子组件destroyed-&gt;父组件destroyed</p>
...
19.2设计图为1920
document.documentElement.style.fontSize = document.documentElement.clientWidth / 19.2 + 'px';</p>
<p>设计图10px;css：0.1rem</p>
<p>pxtorem插件</p>
...
<p>支持链式调用，可以解决回调地狱问题，promise指定回调函数的方式更加灵活</p>
<ol start="2">
<li>什么是回调地狱？</li>
</ol>
<p>回调函数嵌套调用，外部回调函数异步执行结果是嵌套的回调函数的执行条件</p>
<ol start="3">
<li>回调地狱的缺点</li>
</ol>
<p>不便于阅读，不便于异常处理</p>
...
<p>Promise.all()方法接收一个数组作为参数，当数组的所有的Promise都为resolve状态， Promise.all()才会成功，若有一个失败，都会被认为是失败的</p>
...
<ol>
<li>call、apply、bind都是改变this指向的方法，第一个参数都是指定函数内部中this的指向</li>
<li>call和apply都是<code v-pre>立即执行函数</code>，apply第二个参数用数组接受，call，后面可以接收多个参数</li>
<li>bind方法没有立即执行，因为bind返回的是一个函数</li>
</ol>
<h3 id="_10-vue-双向绑定原理" tabindex="-1"><a class="header-anchor" href="#_10-vue-双向绑定原理" aria-hidden="true">#</a> 10. vue 双向绑定原理</h3>
<ol>
<li>通过数据劫持结合发布订阅模式来实现，具体数据劫持通过Object.defineProperty()来实现数据劫持</li>
...
<ol>
<li>var存在<code v-pre>变量提升</code>，可重复声明同一变量，声明的变量均可改</li>
<li>let没有变量提升，不可重复声明同一变量，声明的变量均可改</li>
<li>const没有变量提升，不可重复声明同一变量，声明的基本数据类型不可改，引用类型可改属性，不可只声明变量而不赋值</li>
</ol>
...
<p>includes可以检测NaN，indexOf不能检测NaN，includes内部使用了Number.isNaN对NaN进行了匹配</p>
...
<p>是<code v-pre>generator</code> + <code v-pre>Promise</code>的语法糖，主要的作用是用同步方式执行异步操作，await只能在async函数中使用，async函数执行会返回一个Promise，值由函数的return值所决定</p>
...
<p>是把接受多个参数的函数变换成接受一个单一参数（最初函数的第一个参数）的函数，并且返回接受余下的参数而且返回结果的新函数的技术。</p>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><div class="custom-container tip"><p class="custom-container-title">好处</p>
<ol>
<li>
<p>参数复用</p>
</li>
<li>
<p>延迟执行 其实 Function.prototype.bind 就是科里化的实现例子</p>
</li>
</ol>
</div>
...
<p>其实就是数据，把 dom 变成数据结构,用一个 JS 对象来描述一个真实 DOM 结构,大大节省了浏览器损耗。</p>
...
<p>当虚拟DOM的某个节点发生了变化，会生成一个新的VNode，然后通过新的VNode与之前的旧的VNode进行对比，把不同的地方修改在真实的DOM上，在将旧的VNode更新节点的值。
diff过程的核心就是去使用patch函数，对比新老节点，然后修改真实DOM。</p>
<p><img src="@source/interview/imgs/diff.png" alt="diff"></p>
...
通过监听 <code v-pre>hashchange</code> 事件来监听页面 hash 的变化，通过解析 hash 的值来切换页面。</p>
<p><strong>优点：</strong> 兼容性最佳，无需服务端配置。</p>
<p><strong>缺点：</strong> 服务端无法获取 hash 部分内容, 可能和锚点冲突，SEO 不友好</p>
<p><strong>2. History 模式</strong></p>
<p>History 路由核心主要依赖 History API 里的两个方法和一个事件，其中两个方法用于操作浏览器的历史记录，事件用于监听历史记录的切换</p>
<p><strong>方法：</strong></p>
<p><code v-pre>history.pushState</code>：将给定的 Data 添加到当前标签页的历史记录栈中。
<code v-pre>history.replaceState</code>：将给定的 Data 更新到历史记录栈中最新的一条记录中。</p>
<p><strong>事件：</strong></p>
<p><code v-pre>popstate</code>：监听历史记录的变化。</p>
<p><strong>优点：</strong> 服务端可获取完整的链接和参数。前端监控友好,SEO 相对 Hash 路由友好。</p>
<p><strong>缺点：</strong> 兼容性稍弱,需要服务端额外配置（各 path 均指向同一个 HTML）。</p>
...
<p><img src="@source/interview/imgs/proto.jpg" alt="proto">
<strong>原型链继承</strong>：
让子类的原型指向父类的实例</p>
<p><strong>构造函数继承</strong>：
在子类的构造函数中执行父类的构造函数，改变this的指向</p>
<p><strong>组合式继承</strong>：
把原型链继承和构造函数继承结合起来就形成了组合式继承</p>
<p><strong>ES6 Class的继承</strong>：
Class 可以通过extends关键字实现继承，让子类继承父类的属性和方法。子类必须在constructor()方法中调用super()得到与父类同样的实例属性和方法</p>
...
arr<span class="token punctuation">.</span><span class="token function">forEach</span><span class="token punctuation">(</span><span class="token parameter">v</span> <span class="token operator">=></span> <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">}</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="_33-vue-组件中的-data-为什么必须是函数" tabindex="-1"><a class="header-anchor" href="#_33-vue-组件中的-data-为什么必须是函数" aria-hidden="true">#</a> 33. vue 组件中的 data 为什么必须是函数</h3>
<p>因为每一个 vue 组件都是一个 vue 实例，通过 new Vue() 实例化，引用同一个对象，如果 data 直接是一个对象的话，那么一旦修改其中一个组件的数据，其他组件相同数据就会被改变，而 data 是函数的话，每个 vue 组件的 data 都因为函数有了自己的<code v-pre>作用域</code>，互不干扰。</p>
<p>🎉 💯 💯 💯</p>
</div></template>
